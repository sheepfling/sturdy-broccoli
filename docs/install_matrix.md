...
You can prepend `./bootstrap doctor` before that sequence when you want a
...
| Goal | Command | Notes |
| --- | --- | --- |
| Lean local setup | `./bootstrap python` | Default operator path. Installs the workspace with lean `test` extras. |
...
| --- | --- | --- | --- |
| Pure Python | Python only | `./bootstrap python` | `python examples/target_radar_simulation.py --backend python --steps 5` |
| Java shim | Python only for local shim tests | `./bootstrap python` | `python examples/java_shim_federate.py` |
...
| CERTI | working base Python plus local CERTI runtime | bootstrap Python first, then `./certi-easy preflight` | `./certi-easy smoke compare` |
| Pitch | working base Python plus local Pitch runtime or Docker flow | bootstrap Python first, then `./pitch preflight` | `./pitch smoke` |
