...
|---|---|---|---|---|---|---|---|
| Python RTI | n/a | `python`, `in-memory`, `python-in-memory` | direct Python | direct | green | [test_python_backend_extended_services.py](tests/backends/test_python_backend_extended_services.py), [run_two_federate_suite.py](scripts/run_two_federate_suite.py) | strongest reference path |
...
| CERTI | upstream | no separate backend name | baseline-only | n/a | partial | [vendor_runtime_smoke.sh](scripts/ci/vendor_runtime_smoke.sh), [test_certi_real_backend_matrix.py](tests/vendors/test_certi_real_backend_matrix.py), [certi_runtime_limitations.md](docs/certi_runtime_limitations.md) | selected only via `HLA2010_CERTI_UPSTREAM_*`; used for attribution against patched CERTI |
