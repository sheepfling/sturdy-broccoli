# Clause Index

This sub-index groups the spec-facing clause maps and traceability entry points.

Canonical order:

1. runtime route inventory
2. clause-to-capability map
3. clause-to-conformance map
4. vendor-specific traceability

Primary entry points:

- [../backend_route_inventory.md](../backend_route_inventory.md)
- [../backend_capability_matrix.md](../backend_capability_matrix.md)
- [../backend_conformance_matrix.md](../backend_conformance_matrix.md)
- [../certi_spec_traceability.md](../certi_spec_traceability.md)
- [../rti_options_and_test_matrix.md](../rti_options_and_test_matrix.md)

Use this page when you want to answer: which runtime family owns this clause,
and where is the first reviewable matrix for it?
