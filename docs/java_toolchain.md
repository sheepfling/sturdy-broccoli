...
required for language-shim builds and Java-route tests.
...
- whether the Java standard shim jars already exist
...
The repository prefers a real JDK, not just a runtime, because language-shim builds
need `javac` and `jar` as well as `java`.

## Required Tools

For Java-backed routes and language-shim builds, the useful toolchain is:
...
`./tools/java` is the short operator front door. The longer shim-routes alias
remains `./tools/shim-routes java doctor`.
