...
`hla.fom.mom.create_standard_mim_module()` parses the bundled XML through the same `hla.fom.parse_fom_file()` path used for normal FOM modules. The fallback mini-MIM remains only as a defensive backup for unusual packaging environments.
