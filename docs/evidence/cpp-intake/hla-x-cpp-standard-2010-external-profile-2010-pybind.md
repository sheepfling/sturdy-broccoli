# C++ RTI Core Certification: 2010 pybind

- Status: `adapter-smoke-green`
- Artifact: `hla-x-cpp-standard-2010-external-profile`
- Route: `cpp-2010-sdk-pybind`
- Connect/disconnect: PASS
- Callbacks: PASS
- Core scenario: BLOCKED
- Trace comparison: BLOCKED
- Capsule directory: `/private/tmp/hla-x-cpp-certify-capsules/hla-x-cpp-standard-2010-external-profile`
- Blocked reason: full core scenario through generated C++ SDK runtime capsule is not implemented yet

## Missing Trace Events

- none

## Unsupported Services

- requestFederationSave
- requestFederationRestore
- createRegion

## Errors

- none

## Warnings

- none
