# Evidence

This family stores unpacked verification and analysis packets that were
promoted from `INBOX`.

Use this family for imported evidence and preserved packet contents, not for
newly authored spec summaries.

Canonical order:

1. imported packet
2. extracted analysis artifacts
3. preserved source-context copies
4. archive reference

Internal indexes:

- [packet_index.md](packet_index.md)

Current contents:

- `hla2010_python_verification_evidence_v0_13/`
  imported verification packet with analysis matrices, traceability assets,
  and source-context copies from the supplied code drop

Archive source:

- `../../archives/verification/hla2010_python_verification_evidence_v0_13.zip`

Related families:

- [../verification/README.md](../verification/README.md)
- [../specs/README.md](../specs/README.md)
