# HLA-X MessageTest v0.1 Test Plan

| ID | Test | Federates | Procedure | Pass criteria | Evidence |
|---|---|---|---|---|---|
| MT-001 | FOM load and join | FederationManager, TestDesignFederate, TestExecutionFederate | Create federation with `HLAx_Base.xml` and `HLAx_MessageTest.xml`; join required federates. | All federates join, publish `FederateHealth`, and emit `FederateReady`. | Health log, join log |
