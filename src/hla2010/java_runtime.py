"""Java runtime discovery helpers.

The imported seed assumed ``java``, ``javac``, and ``jar`` were visible on the
host ``PATH``. On this machine the usable JDK comes from ``jdk4py`` instead, so
runtime discovery is centralized here and reused by build/test helpers and real
RTI adapter wiring.
"""
