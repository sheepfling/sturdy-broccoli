...
def build_service_conformance_matrix(*, version: str = "0.13.0") -> dict[str, Any]:
    ...
    The canonical implementation lives in ``hla2010.conformance``; this adapter
    preserves the flatter repo-seed dictionary shape used by the current tests
    and artifact writers.
    """
    from hla2010_repo_internal.conformance import build_service_conformance_matrix as _build
    ...
    for index, row in enumerate(canonical.rows, start=1):
        has_backend = (
            ...
            or row.python_entry_point.startswith("hla2010_rti_python.backend.PythonRTIBackend.")
        )
...
def build_requirements_ledger(*, version: str = "0.13.0") -> dict[str, Any]:
    ...
    from hla2010_repo_internal.conformance import build_requirements_ledger as _build
