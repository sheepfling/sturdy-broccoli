...
from hla2010_repo_internal.conformance_runtime import (
...
from hla2010_repo_internal.mom_negative_testing import build_mom_negative_test_cases
from hla2010_repo_internal.mom_negative_testing import default_mom_model, mom_negative_case_report
...
def _implementation_refs(row: ServiceConformanceRow) -> tuple[str, ...]:
    ...
    if row.interface == "RTIambassador":
        refs.append("packages/hla2010-rti-python/src/hla2010_rti_python/backend.py")
...
def build_service_conformance_matrix(*, version: str = "0.13.0") -> ServiceConformanceMatrix:
    ...
    for method in RTI_METHOD_NAMES:
        ...
        seed_row = ServiceConformanceRow(
            ...
            python_entry_point=(
                f"hla2010_rti_python.backend.PythonRTIBackend.{handler}"
                if has_handler
    ...
    for method in CALLBACK_METHOD_NAMES:
        ...
        evidence = (
            "packages/hla2010-rti-backend-common/src/hla2010_rti_backend_common/recording.py::RecordingFederateAmbassador",
            *callback_evidence,
        )
        asset_id = _section_asset_id(section)
        seed_row = ServiceConformanceRow(
