...
//     main_root.displaySmall();
    ...
    for (const auto& object_class : *ObjectClasses) {
        try {
            ...
            // A module may legally scaffold an existing class like HLAobjectRoot
            // without restating attributes already supplied by the base MIM/root.
            ...
            if (existing_attributes.size() != new_attributes.size()) {
                ...
                std::cerr << "[root-merge] object class '" << existing_object->getName()
                          << "' attribute count differs existing=" << existing_attributes.size()
                          << " new=" << new_attributes.size() << std::endl;
                return false;
            }
            //  names
            ...
            for (const auto& kv : new_attributes) {
                ...
                if (find_it == end(names)) {
                    ...
                    std::cerr << "[root-merge] object class '" << existing_object->getName()
                              << "' new attribute '" << kv.second->getName() << "' not present in existing root"
                              << std::endl;
                    return false;
                }
                else {
            ...
            if (!names.empty()) {
                ...
                std::cerr << "[root-merge] object class '" << existing_object->getName()
                          << "' existing-only attributes remain:";
