    ...
    : my_message{std::move(message)}, maxHandle{maxAttHandle}
{
    ...
    std::cerr << "[broadcastlist] construct type=" << my_message->getMessageName() << " maxHandle=" << maxHandle
              << " sender=" << my_message->getFederate() << std::endl;
...
Responses ObjectClassBroadcastList::preparePendingMessage(SecurityServer& server)
{
    ...
    std::cerr << "[broadcastlist] preparePendingMessage type=" << my_message->getMessageName()
              << " lines=" << my_lines.size() << " maxHandle=" << maxHandle << std::endl;
    switch (my_message->getMessageType()) {
    case NetworkMessage::Type::REFLECT_ATTRIBUTE_VALUES:
    case NetworkMessage::Type::REQUEST_ATTRIBUTE_OWNERSHIP_ASSUMPTION:
...
Responses ObjectClassBroadcastList::preparePendingRAVMessage(SecurityServer& server)
{
    ...
    std::cerr << "[broadcastlist] preparePendingRAVMessage type=" << my_message->getMessageName()
              << " lines=" << my_lines.size() << std::endl;
    ...
    std::cerr << "[broadcastlist] relevantAttributes";
    for (const auto& attr : relevantAttributes) {
        std::cerr << " " << attr;
    }
    std::cerr << std::endl;

    for (auto& line : my_lines) {
        std::cerr << "[broadcastlist] line federate=" << line.getFederate() << " waitingAny="
                  << (line.isWaitingAny(maxHandle) ? 1 : 0) << std::endl;
        ...
        if (line.isWaitingAny(maxHandle)) {
            ...
            if (line.isWaitingAll(relevantAttributes)) {
                ...
                std::cerr << "[broadcastlist] complete message federate=" << line.getFederate() << std::endl;
            }
            else {
                ...
                std::cerr << "[broadcastlist] reduced message federate=" << line.getFederate() << std::endl;
            }
            ...
            try {
                ...
                sockets.push_back(server.getSocketLink(line.getFederate()));
#endif
                std::cerr << "[broadcastlist] queue response federate=" << line.getFederate() << " sockets="
                          << sockets.size() << " msg="
                          << (currentMessage ? currentMessage->getMessageName() : "<null>") << std::endl;
                responses.emplace_back(sockets, std::move(currentMessage));
                std::cerr << "[broadcastlist] queued response federate=" << line.getFederate() << std::endl;
            }
            catch (Exception& e) {
                Debug(D, pdExcept) << "Reference to a killed Federate while broadcasting." << std::endl;
                std::cerr << "[broadcastlist] exception while queuing federate=" << line.getFederate()
                          << " name=" << e.name() << " reason='" << e.reason() << "'" << std::endl;
            }
            ...
            for (unsigned int attrIndex = 1; attrIndex <= maxHandle; attrIndex++) {
                if (line.stateFor(attrIndex) == ObjectBroadcastLine::State::Waiting) {
                    std::cerr << "[broadcastlist] mark sent federate=" << line.getFederate() << " attr=" << attrIndex
                              << std::endl;
            ...
            Debug(D, pdProtocol) << "No message sent to Federate " << line.getFederate() << std::endl;
            std::cerr << "[broadcastlist] no message federate=" << line.getFederate() << std::endl;
        }

        Debug(G, pdGendoc) << "exit  ObjectClassBroadcastList::sendPendingRAVMessage" << std::endl;
    }

    std::cerr << "[broadcastlist] preparePendingRAVMessage complete responses=" << responses.size() << std::endl;
                                                                                ...
                                                                                const ObjectBroadcastLine& line)
{
    ...
    for (uint32_t i = 0; i < message->getAttributesSize(); ++i) {
        ...
        if (line.stateFor(currentAttrib) == ObjectBroadcastLine::State::Waiting) {
            ...
            std::cerr << "[broadcastlist] createResponseMessage include attr=" << currentAttrib
                      << " federate=" << line.getFederate() << std::endl;
        }
    }
...
ObjectClassBroadcastList::createResponseMessageWithValues(T* message, const ObjectBroadcastLine& line)
{
    ...
    for (uint32_t i = 0; i < message->getAttributesSize(); ++i) {
        ...
        if (line.stateFor(currentAttrib) == ObjectBroadcastLine::State::Waiting) {
            ...
            std::cerr << "[broadcastlist] createResponseMessageWithValues include attr=" << currentAttrib
                      << " federate=" << line.getFederate() << std::endl;
        }
    }
    return std::unique_ptr<NetworkMessage>(static_cast<NetworkMessage*>(reducedMessage.release()));
