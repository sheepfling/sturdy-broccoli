...
Responses ObjectClass::broadcastClassMessage(ObjectClassBroadcastList* ocbList, const Object* source)
{
    ...
    std::cerr << "[objectclass-broadcast] enter type=" << static_cast<int>(ocbList->getMsg().getMessageType())
              << " name=" << ocbList->getMsg().getMessageName() << " source="
              << (source ? static_cast<long>(source->getHandle()) : 0) << std::endl;
    ...
    if ((ocbList->getMsg().getMessageType() == NetworkMessage::Type::REQUEST_ATTRIBUTE_OWNERSHIP_ASSUMPTION)) {
        std::cerr << "[objectclass-broadcast] RAOA trim attributes count="
                  << ocbList->getMsgRAOA()->getAttributesSize() << std::endl;
    ...
    case NetworkMessage::Type::REQUEST_ATTRIBUTE_OWNERSHIP_ASSUMPTION: {
        std::cerr << "[objectclass-broadcast] RAOA scan class-attribute-count=" << _handleClassAttributeMap.size()
                  << std::endl;
        ...
        for (HandleClassAttributeMap::iterator i = _handleClassAttributeMap.begin();
             ...
             ++i) {
            std::cerr << "[objectclass-broadcast] update RAOA attr=" << i->first << " name=" << i->second->getName()
                      << std::endl;
    ...
    std::cerr << "[objectclass-broadcast] preparePendingMessage type="
              << static_cast<int>(ocbList->getMsg().getMessageType()) << std::endl;
    auto ret = ocbList->preparePendingMessage(*server);
    std::cerr << "[objectclass-broadcast] preparePendingMessage complete responses=" << ret.size() << std::endl;
...
{
    Responses ret;
    std::cerr << "[objectclass-ownership] negotiatedDivestiture start federate=" << theFederateHandle
              << " object=" << object->getHandle() << " attributes=" << theAttributeList.size() << " tag='"
              << theTag << "'" << std::endl;
    // Pre-conditions checking
    ...
    for (unsigned i = 0; i < theAttributeList.size(); i++) {
        ...
        std::cerr << "[objectclass-ownership] negotiatedDivestiture attr=" << oa->getHandle()
                  << " owner=" << oa->getOwner() << " divesting=" << (oa->beingDivested() ? 1 : 0)
                  << " hasCandidates=" << (oa->hasCandidates() ? 1 : 0) << std::endl;
    ...
    if (server != NULL) {
        ...
        for (uint32_t i = 0; i < theAttributeList.size(); i++) {
            ...
            if (oa->hasCandidates()) {
                ...
                std::cerr << "[objectclass-ownership] negotiatedDivestiture immediate-transfer attr="
                          << oa->getHandle() << " newOwner=" << NewOwner << std::endl;
                ...
                std::cerr << "[objectclass-ownership] negotiatedDivestiture mark-divesting attr=" << oa->getHandle()
                          << std::endl;
                assumption_count++;
            }
        }
        ...
        if (acquisition_count != 0) {
            ...
            std::cerr << "[objectclass-ownership] negotiatedDivestiture send acquisition notifications count="
                      << acquisition_count << std::endl;
        ...
        if (divestiture_count != 0) {
            ...
            std::cerr << "[objectclass-ownership] negotiatedDivestiture send divestiture notification count="
                      << divestiture_count << std::endl;
        ...
        if (assumption_count != 0) {
            ...
            Debug(D, pdProtocol) << "Object " << object->getHandle() << " divestiture in class " << handle
                                 << ", now broadcasting..." << std::endl;
            std::cerr << "[objectclass-ownership] negotiatedDivestiture broadcast assumptions count="
                      << assumption_count << std::endl;
    ...
    std::cerr << "[objectclass-ownership] negotiatedDivestiture done responses=" << ret.size() << std::endl;
...
void ObjectClass::attributeOwnershipAcquisition(FederateHandle theFederateHandle,
                                                ...
                                                const std::string& theTag)
{
    std::cerr << "[objectclass-ownership] acquisition start federate=" << theFederateHandle
              << " object=" << object->getHandle() << " attributes=" << theAttributeList.size() << " tag='"
              << theTag << "'" << std::endl;
	 ...
	 */
        ...
        for (unsigned i = 0; i < theAttributeList.size(); i++) {
            ...
            if ((oldOwner == 0) || (oa->beingDivested())) {
                ...
                oa->setDivesting(false);
                std::cerr << "[objectclass-ownership] acquisition immediate-owner attr=" << oa->getHandle()
                          << " newOwner=" << theFederateHandle << std::endl;
                ...
                std::cerr << "[objectclass-ownership] acquisition request-release attr=" << oa->getHandle()
                          << " oldOwner=" << oldOwner << std::endl;
            }
        }
        if (notification_counter != 0) {
            AnswerNotification->setAttributesSize(notification_counter);
            std::cerr << "[objectclass-ownership] acquisition send notification count=" << notification_counter
                      << std::endl;
        ...
        if (!diffusionDivestiture.empty()) {
            NM_Attribute_Ownership_Divestiture_Notification AODN;
            std::cerr << "[objectclass-ownership] acquisition send divestiture notifications count="
                      << diffusionDivestiture.size() << std::endl;
        ...
        if (!diffusionRelease.empty()) {
            ...
            std::cerr << "[objectclass-ownership] acquisition send release requests count=" << diffusionRelease.size()
                      << std::endl;
                                                                   ...
                                                                   const std::vector<AttributeHandle>& the_attributes)
{
    ...
    for (unsigned i = 0; i < the_attributes.size(); i++) {
        ...
        if (!oa->hasCandidates()) {
            std::cerr << "[objectclass-ownership] releaseResponse missing-candidate attr=" << oa->getHandle()
                      << std::endl;
    ...
    if (server != NULL) {
        ...
        for (unsigned i = 0; i < the_attributes.size(); i++) {
            ...
            newOwner = oa->getFirstCandidate();
            std::cerr << "[objectclass-ownership] releaseResponse transfer attr=" << oa->getHandle()
                      << " oldOwner=" << the_federate << " newOwner=" << newOwner << std::endl;
...
void ObjectClass::attributeOwnershipReleaseDenied(FederateHandle the_federate,
                                                  Object* object,
                                                  const std::vector<AttributeHandle>& the_attributes)
{
    std::cerr << "[objectclass-ownership] releaseDenied start federate=" << the_federate
              << " object=" << object->getHandle() << " attributes=" << the_attributes.size() << std::endl;
    for (unsigned index = 0; index < the_attributes.size(); index++) {
        getAttribute(the_attributes[index]);
    }
    ...
    for (unsigned i = 0; i < the_attributes.size(); i++) {
        ObjectAttribute* oa = object->getAttribute(the_attributes[i]);
        std::cerr << "[objectclass-ownership] releaseDenied clear attr=" << oa->getHandle()
                  << " candidateCount=" << (oa->hasCandidates() ? 1 : 0) << std::endl;
        oa->clearCandidates();
        oa->setDivesting(false);
    }
}
