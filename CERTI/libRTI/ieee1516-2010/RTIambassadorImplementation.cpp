                                                             ...
                                                             rti1516e::RTIinternalError)
{
    ...
    std::cerr << "[certi1516e] createFederationExecution begin federation="
              << std::string{begin(federationExecutionName), end(federationExecutionName)}
              << " fom_count=" << fomModules.size() << std::endl;
    ...
    std::cerr << "[certi1516e] createFederationExecution executeService" << std::endl;
    p->executeService(&req, &rep);
    std::cerr << "[certi1516e] createFederationExecution complete" << std::endl;
                                                                 ...
                                                                 rti1516e::RTIinternalError)
{
    ...
    std::cerr << "[certi1516e] joinFederationExecution begin federate="
              << std::string{begin(federateName), end(federateName)}
              << " type=" << std::string{begin(federateType), end(federateType)}
              << " federation=" << std::string{begin(federationExecutionName), end(federationExecutionName)}
              << std::endl;
    ...
    std::cerr << "[certi1516e] joinFederationExecution executeService" << std::endl;
    p->executeService(&req, &rep);
    std::cerr << "[certi1516e] joinFederationExecution complete" << std::endl;
...
void RTI1516ambassador::attributeOwnershipReleaseDenied(
    ...
    rti1516e::AttributeHandleSet const& theAttributes) throw(rti1516e::AttributeNotOwned,
                                                                 ...
                                                                 rti1516e::RTIinternalError)
{
    Debug(D, pdDebug) << "1516e attributeOwnershipReleaseDenied using local denial path for object "
                      << rti1516e::ObjectInstanceHandleFriend::toCertiHandle(theObject) << " attrs="
                      << theAttributes.size() << std::endl;
    M_Attribute_Ownership_Release_Denied req, rep;
