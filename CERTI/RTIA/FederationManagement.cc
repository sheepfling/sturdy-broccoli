                                                     ...
                                                     Exception::Type& e)
{
    ...
    std::cerr << "[rtia_fm] createFederationExecution federation=" << federation_execution_name
              << " modules=" << fom_module_designators.size() << " mim=" << mim_designator
              << " rti_version=" << static_cast<int>(rti_version) << std::endl;
    ...
    std::cerr << "[rtia_fm] send CREATE_FEDERATION_EXECUTION to RTIG" << std::endl;
    ...
    std::cerr << "[rtia_fm] recv CREATE_FEDERATION_EXECUTION from RTIG exception="
              << static_cast<int>(response->getException()) << " reason='" << response->getExceptionReason() << "'"
              << std::endl;
                                                             ...
                                                             Exception::Type& e)
{
    ...
    std::cerr << "[rtia_fm] joinFederationExecution federate=" << federate_name << " type=" << federate_type
              << " federation=" << federation_execution_name << " additional_modules="
              << additional_fom_modules.size() << " rti_version=" << static_cast<int>(rti_version) << std::endl;
    ...
    std::cerr << "[rtia_fm] send JOIN_FEDERATION_EXECUTION to RTIG" << std::endl;

    comm->sendMessage(&request);

    // Waiting RTIG answer
    std::unique_ptr<NetworkMessage> response(comm->waitMessage(NetworkMessage::Type::JOIN_FEDERATION_EXECUTION, 0));
    std::cerr << "[rtia_fm] recv JOIN_FEDERATION_EXECUTION from RTIG exception="
              << static_cast<int>(response->getException()) << " reason='" << response->getExceptionReason() << "'"
              << std::endl;
