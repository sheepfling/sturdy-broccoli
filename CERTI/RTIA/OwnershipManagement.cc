...
void OwnershipManagement::attributeOwnershipReleaseDenied(ObjectHandle theObject,
                                                          const std::vector<AttributeHandle>& attribArray,
                                                          uint32_t attribArraySize,
                                                          Exception::Type& e)
{
    NM_Attribute_Ownership_Release_Denied req;
    ...
    Debug(D, pdDebug) << "RELEASE_DENIED Object " << theObject << " handleArraySize " << req.getAttributesSize()
                      << std::endl;

    comm->sendMessage(&req);

    std::unique_ptr<NetworkMessage> rep(
        comm->waitMessage(NetworkMessage::Type::ATTRIBUTE_OWNERSHIP_RELEASE_DENIED, req.getFederate()));
