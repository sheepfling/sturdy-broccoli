...
void RTIA::execute()
{
    ...
    while (fm.getConnectionState() != FederationManagement::ConnectionState::Ended) {
        ...
        case Communications::ReadResult::FromNetwork:
            std::cerr << "[rtia-loop] dispatch network msg="
                      << (msgFromRTIG ? msgFromRTIG->getMessageName() : "<null>")
                      << " type="
                      << (msgFromRTIG ? static_cast<int>(msgFromRTIG->getMessageType()) : -1) << std::endl;
        ...
        case Communications::ReadResult::FromFederate:
            std::cerr << "[rtia-loop] dispatch federate msg="
                      << (msgFromFederate ? msgFromFederate->getMessageName() : "<null>")
                      << " type="
                      << (msgFromFederate ? static_cast<int>(msgFromFederate->getMessageType()) : -1)
                      << std::endl;
