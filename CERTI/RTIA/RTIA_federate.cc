...
void RTIA::chooseFederateProcessing(Message* request, Message* answer, Exception::Type& e)
{
    ...
    case Message::CREATE_FEDERATION_EXECUTION_V4: {
        ...
        std::cerr << "[rtia] handle CREATE_FEDERATION_EXECUTION_V4 federation="
                  << CFEq->getFederationExecutionName() << " modules=" << CFEq->getFomModuleDesignatorsSize()
                  << " mim='" << CFEq->getMimDesignator() << "'" << std::endl;
        
        // Store FEDid for future usage (JOIN_FEDERATION_EXECUTION) into fm
        fm.createFederationExecution(
            CFEq->getFederationExecutionName(), CFEq->getFomModuleDesignators(), CFEq->getMimDesignator(), CFEq->getRtiVersion(), e);
        std::cerr << "[rtia] createFederationExecution returned exception=" << static_cast<int>(e) << std::endl;
    ...
    case Message::JOIN_FEDERATION_EXECUTION_V4: {
        ...
        std::cerr << "[rtia] handle JOIN_FEDERATION_EXECUTION_V4 federate=" << JFEq->getFederateName()
                  << " type=" << JFEq->getFederateType() << " federation=" << JFEq->getFederationExecutionName()
                  << " additional_modules=" << JFEq->getAdditionalFomModulesSize() << std::endl;
        ...
        JFEr->setFederate(fm.joinFederationExecution(JFEq->getFederateName(),
                                                     ...
                                                     &my_root_object,
                                                     e));
        std::cerr << "[rtia] joinFederationExecution returned exception=" << static_cast<int>(e) << std::endl;
    ...
    case Message::ATTRIBUTE_OWNERSHIP_RELEASE_DENIED: {
        M_Attribute_Ownership_Release_Denied* AORDq;
        AORDq = static_cast<M_Attribute_Ownership_Release_Denied*>(request);

        Debug(D, pdTrace) << "Receiving Message from Federate, "
                             "type attributeOwnershipReleaseDenied."
                          << std::endl;
        Debug(D, pdTrace) << "Object " << AORDq->getObject() << " nb Attribute " << AORDq->getAttributesSize()
                          << std::endl;

        owm.attributeOwnershipReleaseDenied(AORDq->getObject(), AORDq->getAttributes(), AORDq->getAttributesSize(), e);
    } break;
...
void RTIA::processFederateRequest(Message* request)
{
    ...
    std::cerr << "[rtia-federate] process request msg="
              << (request ? request->getMessageName() : "<null>") << " type="
              << (request ? static_cast<int>(request->getMessageType()) : -1) << " state="
              << static_cast<int>(fm.getConnectionState()) << std::endl;
    ...
    try {
        ...
        case FederationManagement::ConnectionState::Ready: {
            ...
            std::cerr << "[rtia-federate] chooseFederateProcessing completed exc=" << static_cast<int>(exc)
                      << std::endl;
    ...
    //  rep->setException(static_cast<Exception::Type>(e.getMessageType()),e.reason());
        ...
        Debug(D, pdExcept) << "Caught Exception: " << e.name() << std::endl;
        std::cerr << "[rtia] CouldNotOpenFED reason='" << e.reason() << "'" << std::endl;
        ...
        Debug(D, pdExcept) << "Caught Exception: " << e.name() << std::endl;
        std::cerr << "[rtia] ErrorReadingFED reason='" << e.reason() << "'" << std::endl;
        ...
        Debug(D, pdExcept) << "Caught Exception: " << e.name() << std::endl;
        std::cerr << "[rtia] FederationExecutionAlreadyExists reason='" << e.reason() << "'" << std::endl;
        rep->setException(Exception::Type::FederationExecutionAlreadyExists, e.reason());
    }
    catch (FederationExecutionDoesNotExist& e) {
        Debug(D, pdExcept) << "Caught Exception: " << e.name() << std::endl;
        std::cerr << "[rtia] FederationExecutionDoesNotExist reason='" << e.reason() << "'" << std::endl;
        ...
        std::cerr << "[rtia] RTIinternalError reason='" << e.reason() << "'" << std::endl;
        if (!e.reason().empty()) {
        ...
        Debug(D, pdExcept) << "Caught Exception: " << e.name() << std::endl;
        rep->setException(Exception::Type::RTIinternalError, e.reason());
    }
    ...
    if (rep->getMessageType() != Message::TICK_REQUEST && rep->getMessageType() != Message::TICK_REQUEST_NEXT
        ...
        std::cerr << "[rtia-federate] send reply msg=" << rep->getMessageName() << " type="
                  << static_cast<int>(rep->getMessageType()) << " exception="
                  << static_cast<int>(rep->getExceptionType()) << " reason='"
                  << (rep->getExceptionReason() ? rep->getExceptionReason() : "") << "'" << std::endl;
        comm.sendUN(rep.get());
