                                 ...
                                 struct timeval* timeout)
{
    ...
    if (msg_reseau && !waitingList.empty()) {
        ...
        *msg_reseau = waitingList.front();
        ...
        std::cerr << "[rtia-comm] waiting-list network msg="
                  << ((*msg_reseau) ? (*msg_reseau)->getMessageName() : "<null>") << " type="
                  << ((*msg_reseau) ? static_cast<int>((*msg_reseau)->getMessageType()) : -1) << std::endl;
    }
        ...
        *msg_reseau = NM_Factory::receive(socketTCP);
        n = ReadResult::FromNetwork;
        std::cerr << "[rtia-comm] immediate tcp network msg="
                  << ((*msg_reseau) ? (*msg_reseau)->getMessageName() : "<null>") << " type="
                  << ((*msg_reseau) ? static_cast<int>((*msg_reseau)->getMessageType()) : -1) << std::endl;
    }
        ...
        *msg_reseau = NM_Factory::receive(socketUDP);
        n = ReadResult::FromNetwork;
        std::cerr << "[rtia-comm] immediate udp network msg="
                  << ((*msg_reseau) ? (*msg_reseau)->getMessageName() : "<null>") << " type="
                  << ((*msg_reseau) ? static_cast<int>((*msg_reseau)->getMessageType()) : -1) << std::endl;
    }
        ...
        *msg = M_Factory::receive(socketUN);
        ...
        std::cerr << "[rtia-comm] immediate unix federate msg="
                  << ((*msg) ? (*msg)->getMessageName() : "<null>") << " type="
                  << ((*msg) ? static_cast<int>((*msg)->getMessageType()) : -1) << std::endl;
    }
    else {
...
// At least one message has been received, read this message.
        ...
        if (FD_ISSET(socketTCP->returnSocket(), &fdset)) {
            ...
            *msg_reseau = NM_Factory::receive(socketTCP);
            n = ReadResult::FromNetwork;
            std::cerr << "[rtia-comm] select tcp network msg="
                      << ((*msg_reseau) ? (*msg_reseau)->getMessageName() : "<null>") << " type="
                      << ((*msg_reseau) ? static_cast<int>((*msg_reseau)->getMessageType()) : -1) << std::endl;
        }
            ...
            *msg_reseau = NM_Factory::receive(socketUDP);
            n = ReadResult::FromNetwork;
            std::cerr << "[rtia-comm] select udp network msg="
                      << ((*msg_reseau) ? (*msg_reseau)->getMessageName() : "<null>") << " type="
                      << ((*msg_reseau) ? static_cast<int>((*msg_reseau)->getMessageType()) : -1) << std::endl;
        }
            ...
            *msg = M_Factory::receive(socketUN);
            n = ReadResult::FromFederate;
            std::cerr << "[rtia-comm] select unix federate msg="
                      << ((*msg) ? (*msg)->getMessageName() : "<null>") << " type="
                      << ((*msg) ? static_cast<int>((*msg)->getMessageType()) : -1) << std::endl;
        }
        else {
