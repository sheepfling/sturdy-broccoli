    ...
    : my_handle(federation_handle), my_name(federation_name), my_rti_version{rti_version}
{
    ...
    if(getRtiVersion() == IEEE_1516_2010) { // mim and modules
        std::cerr << "[rtig_fed] create federation name=" << my_name << " rti_version=2010 modules="
                  << fom_modules.size() << " mim='" << mim_module << "'" << std::endl;
        if (mim_module.empty()) {
                                                    ...
                                                    )
{
    ...
    std::cerr << "[rtig_fed] join federation name=" << my_name << " federate=" << federate_name
              << " additional_modules=" << additional_fom_modules.size() << std::endl;
...
{
    check(federate_handle);
    std::cerr << "[rtig-federation] negotiateDivestiture federate=" << federate_handle << " object=" << object_handle
              << " attributes=" << attribs.size() << " tag='" << tag << "'" << std::endl;

    // Get the object pointer by id from the root object
    Object* object = my_root_object->objects->getObject(object_handle);

    // It may throw *NotDefined
    auto responses = my_root_object->ObjectClasses->negotiatedAttributeOwnershipDivestiture(
        federate_handle, object, attribs, tag);
    std::cerr << "[rtig-federation] negotiateDivestiture responses=" << responses.size() << std::endl;
    return responses;
}
...
{
    check(federate_handle);
    std::cerr << "[rtig-federation] acquire federate=" << federate_handle << " object=" << object_handle
              << " attributes=" << attributes.size() << " tag='" << tag << "'" << std::endl;
    ...
    std::cerr << "[rtig-federation] acquire complete" << std::endl;
}
                                               ...
                                               const vector<AttributeHandle>& attributes)
{
    ...
    std::cerr << "[rtig-federation] respondRelease federate=" << federate_handle << " object=" << object_handle
...
void Federation::denyRelease(FederateHandle federate_handle,
                             ObjectHandle object_handle,
                             const vector<AttributeHandle>& attributes)
{
    check(federate_handle);
    std::cerr << "[rtig-federation] denyRelease federate=" << federate_handle << " object=" << object_handle
              << " attributes=" << attributes.size() << std::endl;
    Object* object = my_root_object->objects->getObject(object_handle);
    my_root_object->ObjectClasses->attributeOwnershipReleaseDenied(federate_handle, object, attributes);
    std::cerr << "[rtig-federation] denyRelease complete" << std::endl;
}
