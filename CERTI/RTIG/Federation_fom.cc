...
void Federation::openFomModules(std::vector<std::string> modules, const bool is_mim)
{
    ...
    try {
        for (auto& module : modules) {
            ...
            std::cerr << "[rtig_fom] open module='" << module << "' is_mim=" << (is_mim ? 1 : 0) << std::endl;

            Debug(D, pdDebug) << "  Find file" << std::endl;
            auto module_path = filepathOf(module);
            std::cerr << "[rtig_fom] resolved path='" << module_path << "'" << std::endl;

            Debug(D, pdDebug) << "  Check file type" << std::endl;
            auto file_type = checkFileType(module_path);
            std::cerr << "[rtig_fom] file type=" << (file_type == FileType::Xml ? "xml" : "fed") << std::endl;
            ...
            if(is_mim) {
                ...
                if (!is_compliant) {
                    std::cerr << "[rtig_fom] invalid MIM module path='" << module_path << "'" << std::endl;
                ...
                if (!is_valid) {
                    std::cerr << "[rtig_fom] invalid FOM module path='" << module_path << "'" << std::endl;
            ...
            std::cerr << "[rtig_fom] module loaded path='" << module_path << "'" << std::endl;
        }
    }
    /*catch (CouldNotOpenFED& e) {
        ...
        Debug(D, pdExcept) << "Caught exception " << e.name() << " : " << e.reason() << std::endl;
        std::cerr << "[rtig_fom] exception name=" << e.name() << " reason='" << e.reason() << "'" << std::endl;
        throw;
    }
