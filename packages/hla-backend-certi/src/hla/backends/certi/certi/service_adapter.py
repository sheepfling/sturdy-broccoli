...
class CERTIBackend(RTIBackend):
            ...
            case "requestFederationSave":
                if len(args) >= 2 and args[1] is not None:
                    ...
                    return self._request_value(
                        ...
                        request_args[0],
                        logical_time_name(request_args[1]),
                        coerce_time_scalar(request_args[1]),
                    )
                request_args = cast(tuple[Any, ...], args)
                return self._request_value("REQUEST_FEDERATION_SAVE", request_args[0])
        ...
        match invocation.method_name:
            ...
            case "changeAttributeOrderType":
                ...
                return self._request_value(
                    ...
                    change_args[0].value,
                    handle_set_spec(change_args[1]),
                    change_args[2].name if isinstance(change_args[2], OrderType) else change_args[2],
                )
            case "sendInteraction":
                if len(args) >= 4:
                    ...
                    parts = self._request_parts(
                        ...
                        logical_time_name(send_args[3]),
                        coerce_time_scalar(send_args[3]),
                    )
                    if not parts:
                        return None
                    handle_id = int(parts[0])
    ...
    def _request_parts(self, command: str, *fields: Any, timeout: float | None = None) -> list[str]:
        ...
        try:
            ...
            exc_type = getattr(__import__("hla.rti1516e.exceptions", fromlist=[exc_name]), exc_name, RTIexception)
