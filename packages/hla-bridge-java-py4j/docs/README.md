# hla-bridge-java-py4j docs

This package owns the installable generic Py4J Java bridge used by vendor
backend packages.

Key owned surfaces:
- `hla.bridges.java.py4j.runtime`: Py4J gateway runtime helpers.
- `hla.bridges.java.py4j.adapter`: the generic Py4J-backed RTI backend
  adapter and proxy helpers.
- `hla.bridges.java.py4j.factory` and `.plugin`: bridge factory and generic
