...
- The shared harness now contains an explicit lost-federate observer scenario:
  - `packages/hla2010-verification-harness/src/hla2010_verification_harness/scenario_lost_federate.py`
- The Python backend can execute that scenario with a test-only injected non-orderly loss path:
  - `packages/hla2010-rti-python/src/hla2010_rti_python/backend.py::force_federate_loss`
