...
The old `hla.vendors.pitch.py4j.{runtime,adapter,factory}` modules remain as
package-local compatibility facades. The removed root
`hla.rti1516e.backends.py4j` modules do not remain available.
...
Import the canonical implementation from `hla.vendors.pitch.py4j`.
