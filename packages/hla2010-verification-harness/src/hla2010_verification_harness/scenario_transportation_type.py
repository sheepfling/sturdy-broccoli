...
def run_transportation_type_scenario(
    ...
    *,
    ...
    attribute = owner_rti.getAttributeHandle(object_class, config.attribute_name)
    interaction = owner_rti.getInteractionClassHandle(config.interaction_class_name)
    transport = owner_rti.getTransportationTypeHandle(config.transportation_name)

    owner_rti.publishObjectClassAttributes(object_class, {attribute})
    owner_rti.publishInteractionClass(interaction)
    ...
    owner_rti.requestAttributeTransportationTypeChange(object_instance, {attribute}, transport)
    owner_rti.queryAttributeTransportationType(object_instance, attribute)
    owner_rti.requestInteractionTransportationTypeChange(interaction, transport)
    owner_rti.queryInteractionTransportationType(interaction)
...
def run_transportation_type_restore_persistence_scenario(
    ...
    *,
    ...
    object_class = owner_rti.getObjectClassHandle(config.object_class_name)
    reliable_attribute = owner_rti.getAttributeHandle(object_class, config.attribute_name)
    best_effort_attribute = owner_rti.getAttributeHandle(object_class, config.second_attribute_name)
    observer_object_class = observer_rti.getObjectClassHandle(config.object_class_name)
    observer_reliable_attribute = observer_rti.getAttributeHandle(observer_object_class, config.attribute_name)
    observer_best_effort_attribute = observer_rti.getAttributeHandle(observer_object_class, config.second_attribute_name)
    interaction = owner_rti.getInteractionClassHandle(config.interaction_class_name)
    observer_interaction = observer_rti.getInteractionClassHandle(config.interaction_class_name)
    parameter = owner_rti.getParameterHandle(interaction, config.parameter_name)
    observer_parameter = observer_rti.getParameterHandle(observer_interaction, config.parameter_name)
    reliable_transport = owner_rti.getTransportationTypeHandle(config.reliable_transportation_name)
    best_effort_transport = owner_rti.getTransportationTypeHandle(config.transportation_name)

    owner_rti.publishObjectClassAttributes(object_class, {reliable_attribute, best_effort_attribute})
    observer_rti.subscribeObjectClassAttributes(observer_object_class, {observer_reliable_attribute, observer_best_effort_attribute})
    owner_rti.publishInteractionClass(interaction)
    observer_rti.subscribeInteractionClass(observer_interaction)
    drain_callbacks_pair(owner_rti, observer_rti, loops=8)
    ...
    owner_rti.requestAttributeTransportationTypeChange(object_instance, {reliable_attribute}, reliable_transport)
    owner_rti.requestAttributeTransportationTypeChange(object_instance, {best_effort_attribute}, best_effort_transport)
    owner_rti.requestInteractionTransportationTypeChange(interaction, best_effort_transport)
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
    ...
    owner_rti.requestFederationSave(config.save_name)
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
    ...
    owner_rti.federateSaveBegun()
    observer_rti.federateSaveBegun()
    owner_rti.federateSaveComplete()
    observer_rti.federateSaveComplete()
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
    ...
    owner_rti.requestFederationRestore(config.save_name)
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
    ...
    owner_rti.federateRestoreComplete()
    observer_rti.federateRestoreComplete()
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
    ...
    owner_federate.clear()
    observer_federate.clear()
    owner_rti.queryAttributeTransportationType(object_instance, reliable_attribute)
    owner_rti.queryAttributeTransportationType(object_instance, best_effort_attribute)
    owner_rti.queryInteractionTransportationType(interaction)
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
...
def run_transportation_type_rejection_scenario(
    ...
    *,
    ...
    with _expect_exception(ObjectInstanceNotKnown):
        owner_rti.requestAttributeTransportationTypeChange(invalid_object_instance, {attribute}, reliable_transport)
    with _expect_exception(AttributeNotDefined):
        owner_rti.requestAttributeTransportationTypeChange(object_instance, {invalid_attribute}, reliable_transport)
    with _expect_exception(InvalidTransportationType):
        owner_rti.requestAttributeTransportationTypeChange(object_instance, {attribute}, invalid_transport)
    with _expect_exception(AttributeNotOwned):
        observer_rti.requestAttributeTransportationTypeChange(object_instance, {observer_attribute}, reliable_transport)
    with _expect_exception(ObjectInstanceNotKnown):
        owner_rti.queryAttributeTransportationType(invalid_object_instance, attribute)
    with _expect_exception(AttributeNotDefined):
        owner_rti.queryAttributeTransportationType(object_instance, invalid_attribute)
    with _expect_exception(InvalidInteractionClassHandle):
        owner_rti.requestInteractionTransportationTypeChange(invalid_interaction, reliable_transport)
    with _expect_exception(InvalidTransportationType):
        owner_rti.requestInteractionTransportationTypeChange(interaction, invalid_transport)
    with _expect_exception(InvalidInteractionClassHandle):
        owner_rti.queryInteractionTransportationType(invalid_interaction)
    drain_callbacks_pair(owner_rti, observer_rti, loops=8)
    ...
    with _expect_exception(SaveInProgress):
        owner_rti.requestAttributeTransportationTypeChange(object_instance, {attribute}, reliable_transport)
    with _expect_exception(SaveInProgress):
        owner_rti.queryAttributeTransportationType(object_instance, attribute)
    with _expect_exception(SaveInProgress):
        owner_rti.requestInteractionTransportationTypeChange(interaction, reliable_transport)
    with _expect_exception(SaveInProgress):
        owner_rti.queryInteractionTransportationType(interaction)
    ...
    owner_rti.abortFederationRestore()
    drain_callbacks_pair(owner_rti, observer_rti, loops=16)
