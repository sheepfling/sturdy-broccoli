...
- A current `2026-06-11` run of `./tools/pitch lost-federate-probe` on this Codex surface does not reach the runtime probe at all: `artifacts/preflight_artifacts/pitch-preflight.json` reports `environment: docker-blocked`, with Docker unreachable and both required loopback ports (`127.0.0.1:8989`, `127.0.0.1:15164`) blocked as `Operation not permitted`; the paired lane summary/report in `artifacts/vendor_runtime_status/vendor_green_pitch_lost_federate_probe/` classify the canonical operator path as `environment-blocked`.
