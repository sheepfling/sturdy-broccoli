...
This package owns:
- Portico runtime discovery and plugin code under `src/hla.vendors.portico/`
- package-owned verification/preflight policy in `src/hla.vendors.portico/testing_policy.py`
...
- `tests/test_rti_portico_split_package.py`: package policy and import boundary guard
