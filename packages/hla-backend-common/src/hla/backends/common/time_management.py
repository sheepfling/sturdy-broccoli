...
This module is deliberately independent from concrete RTI backend packages.
...
@dataclass
class TSOMessage:
    """Normalized timestamp-order message used by algorithm-level tests.

    Backend queues may use richer message objects; the queue helpers below only
    require the common timestamp/recipient/sequence/retraction fields.
    """
    ...
    receive_order_type: Any | None = None
    delivered: bool = False
    payload: Any | None = None


@dataclass
class TSOMessageQueue:
    """Small explicit queue API for timestamp-order delivery decisions."""

    messages: list[Any] = field(default_factory=list)

    def enqueue(self, message: Any) -> Any:
        self.messages.append(message)
        return message

    def active_messages(self, recipient: Any | None = None) -> tuple[Any, ...]:
        return tuple(_sort_tso_messages(_filter_tso_messages(self.messages, recipient)))

    def peek_deliverable(
        self,
        *,
        recipient: Any | None,
        boundary: Any,
        ...
        deliverable = self.list_deliverable_through(
            recipient=recipient,
            boundary=boundary,
            inclusive=inclusive,
            earliest_only=True,
    ...
    def list_deliverable_through(
        ...
        *,
        ...
        selected = [
            msg
            for msg in self.active_messages(recipient)
            if _timestamp_within_boundary(_message_timestamp(msg), boundary, inclusive=inclusive)
        ]
        if earliest_only and selected:
            first_time = _message_timestamp(selected[0])
            selected = [msg for msg in selected if time_eq(_message_timestamp(msg), first_time)]
        return tuple(selected)

    def pop_deliverable_through(
        self,
        *,
        recipient: Any | None,
        boundary: Any,
        ...
        ids = {id(msg) for msg in deliverable}
        self.messages[:] = [msg for msg in self.messages if id(msg) not in ids]
        for msg in deliverable:
            _set_message_flag(msg, "delivered", True)
        return deliverable

    def retract(self, retraction_handle: Any) -> bool:
        for msg in self.messages:
            if _message_retraction_handle(msg) != retraction_handle:
                continue
            if getattr(msg, "delivered", False) or getattr(msg, "retracted", False):
                return False
            _set_message_flag(msg, "retracted", True)
            return True
        return False
...
def _validate_lookahead(lookahead: Any) -> None:
    if time_key(lookahead) < 0:
...
def _federate_is_active(state: Any) -> bool:
    if getattr(state, "resigned", False) or getattr(state, "deleted", False):
        return False
    if hasattr(state, "federation") and getattr(state, "federation") is None:
        return False
    return True
...
def _required_time_field(federate: Any, field_name: str) -> Any:
    value = getattr(federate, field_name, None)
    if value is None:
        raise ValueError(f"time-management federate state is missing {field_name}")
    return value
...
def _message_timestamp(msg: Any) -> Any:
    return getattr(msg, "timestamp")


def _message_recipient(msg: Any) -> Any | None:
    return getattr(msg, "recipient", None)


def _message_retraction_handle(msg: Any) -> Any | None:
    return getattr(msg, "retraction_handle", None)


def _set_message_flag(msg: Any, name: str, value: bool) -> None:
    try:
...
def _message_matches_recipient(msg: Any, recipient: Any | None) -> bool:
    if recipient is None:
        return True
    recipient_handle = _federate_handle(recipient)
    expected = recipient_handle if recipient_handle is not None else recipient
    if not hasattr(msg, "recipient"):
        return True
    return _message_recipient(msg) == expected


def _filter_tso_messages(messages: Iterable[Any], recipient: Any | None = None) -> list[Any]:
    return [
        msg
        for msg in messages
        if not getattr(msg, "retracted", False)
        and not getattr(msg, "delivered", False)
        and _message_matches_recipient(msg, recipient)
    ]


def _sort_tso_messages(messages: Iterable[Any]) -> list[Any]:
    return sorted(messages, key=lambda msg: (time_key(_message_timestamp(msg)), _message_sequence(msg)))


def _timestamp_within_boundary(timestamp: Any, boundary: Any, *, inclusive: bool) -> bool:
    return time_le(timestamp, boundary) if inclusive else time_lt(timestamp, boundary)


def queued_tso_messages(federation: Any, recipient: Any | None = None) -> list[Any]:
    """Return active TSO messages sorted by ``(timestamp, sequence)``."""
    ...
    return list(TSOMessageQueue(list(unique.values())).active_messages(recipient))
    ...
    *,
    ...
    for other in getattr(federation, "federates", {}).values():
        if other is federate and not include_self:
            continue
        if not _federate_is_active(other):
            continue
...
def compute_lits(
    ...
    *,
    ...
    candidates.extend(_message_timestamp(msg) for msg in queued_tso_messages(federation, federate))
    if not candidates:
        return TimeQueryReturn(False, None)
    return TimeQueryReturn(True, time_min(candidates))
...
def deliverable_messages_for_request(federation: Any, federate: Any, request: Any) -> tuple[Any, ...]:
    ...
    queue = TSOMessageQueue(queued_tso_messages(federation, federate))
    requested = _request_time(request)
    mode = _request_mode(request)
    if requested is None or mode is None:
        return ()
    if mode in {FQR, "flushQueueRequest"}:
        return queue.list_deliverable_through(recipient=federate, boundary=requested)
    ...
    if mode in {NMR, NMRA, "nextMessageRequest", "nextMessageRequestAvailable"}:
        return queue.list_deliverable_through(
            recipient=federate,
            boundary=requested,
            earliest_only=True,
        )
...
def compute_grant_decision(
    ...
    *,
    ...
    if mode not in ALL_ADVANCE_MODES:
        return TimeGrantDecision(False, reason=f"unknown time advance mode: {mode}")
    current = getattr(federate, "current_time", None)
    if current is not None and time_lt(requested, current):
        return TimeGrantDecision(False, reason="requested time is before current time")

    messages = deliverable_messages_for_request(federation, federate, request)
    if mode in {FQR, "flushQueueRequest"}:
        optimistic_candidates = [requested]
        if messages:
            optimistic_candidates.append(_message_timestamp(messages[0]))
        ...
        grant_time = time_min(candidates)
        deliverable = tuple(
            msg
            for msg in messages
            if time_le(_message_timestamp(msg), grant_time)
        )
        return TimeGrantDecision(True, grant_time, optimistic, deliverable)
        ...
        if mode in {NMR, "nextMessageRequest", NMRA, "nextMessageRequestAvailable"}:
            ...
            if galt.time_is_valid and galt.time is not None:
                if mode in {NMR, "nextMessageRequest"}:
                    eligible = tuple(msg for msg in messages if time_lt(_message_timestamp(msg), galt.time))
                else:
                    eligible = tuple(msg for msg in messages if time_le(_message_timestamp(msg), galt.time))
                if eligible:
                    ts = _message_timestamp(eligible[0])
    ...
    if mode in {NMR, NMRA, "nextMessageRequest", "nextMessageRequestAvailable"} and messages:
        grant_time = _message_timestamp(messages[0])
...
def valid_tso_lower_bound(federate: Any, *, factory: Any | None = None) -> Any | None:
...
__all__ = [
    ...
    "_federate_is_active",
    "_message_timestamp",
    "_message_retraction_handle",
    "_filter_tso_messages",
    "_sort_tso_messages",
