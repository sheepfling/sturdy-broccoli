...
def _make_backend_factory(backend: str, options: Mapping[str, Any]) -> Any:
    if backend in {"java-shim-jpype", "java-shim-py4j"}:
        profile = "jpype" if backend.endswith("jpype") else "py4j"
        ...
        return factory
    return make_target_radar_factory(backend, backend_options=dict(options))
...
def run_target_radar_backend_matrix(
    ...
    *,
    ...
    for backend in backends:
        ...
        try:
            result = run_target_radar_scenario(_make_backend_factory(backend, options), federation_name=federation_name, steps=target_radar_steps, dt=dt)
