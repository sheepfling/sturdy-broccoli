...
`hla-backend-shim` is deprecated. It remains available only as a legacy
compatibility shim for `rti1516_2025`, but it no
...
This package exists to preserve compatibility-facing entry points
...
surface is retained only for explicit legacy import compatibility coverage.
`runtime_aliases.py` remains as the explicit hatch for
...
you intentionally want the legacy compatibility lane.

Use `python1516_2025` when you want the direct 2025 backend lane.
Use `hla.backends.shim` only when you need the legacy compatibility imports.
