"""Callback decoding for transport-hosted RTI routes."""
...
from .transport_codecs import decode_bytes, decode_handle_set, decode_handle_value_map, decode_order
...
def decode_logical_time(type_name: str, raw: Any) -> Any:
    ...
    if type_name == "HLAfloat64Time":
        return HLAfloat64Time(float(raw))
    raise RTIinternalError(f"Unsupported logical time type from hosted callback: {type_name}")


def dispatch_hosted_callback(ambassador: Any, parts: list[str], *, logical_time_hint: str | None = None) -> None:
    ...
    if kind == "REPORT_FEDERATION_EXECUTIONS":
        ...
        if len(parts) >= 2 and parts[1]:
            for item in parts[1].split(";"):
                ...
                infos.append(FederationExecutionInformation(federation_name, logical_time_name or None))
    ...
    if kind == "TIME_REGULATION_ENABLED":
        invoke_federate_callback(ambassador, "timeRegulationEnabled", decode_logical_time(logical_time_hint or parts[1], parts[2]))
        return
    if kind == "TIME_CONSTRAINED_ENABLED":
        invoke_federate_callback(ambassador, "timeConstrainedEnabled", decode_logical_time(logical_time_hint or parts[1], parts[2]))
        return
    if kind == "TIME_ADVANCE_GRANT":
        invoke_federate_callback(ambassador, "timeAdvanceGrant", decode_logical_time(logical_time_hint or parts[1], parts[2]))
        return
    ...
    if kind == "START_REGISTRATION_FOR_OBJECT_CLASS":
        invoke_federate_callback(ambassador, "startRegistrationForObjectClass", ObjectClassHandle(int(parts[1])))
        return
    if kind == "STOP_REGISTRATION_FOR_OBJECT_CLASS":
        invoke_federate_callback(ambassador, "stopRegistrationForObjectClass", ObjectClassHandle(int(parts[1])))
        return
    if kind == "TURN_INTERACTIONS_ON":
        invoke_federate_callback(ambassador, "turnInteractionsOn", InteractionClassHandle(int(parts[1])))
        return
    if kind == "TURN_INTERACTIONS_OFF":
        invoke_federate_callback(ambassador, "turnInteractionsOff", InteractionClassHandle(int(parts[1])))
        return
    ...
    if kind == "REMOVE_OBJECT_INSTANCE":
        if len(parts) >= 5:
            ...
            return
        invoke_federate_callback(ambassador, "removeObjectInstance", ObjectInstanceHandle(int(parts[1])), decode_bytes(parts[2]))
        return
    ...
    if kind == "ANNOUNCE_SYNC_POINT":
        invoke_federate_callback(ambassador, "announceSynchronizationPoint", parts[1], decode_bytes(parts[2]))
        return
    ...
    if kind == "INITIATE_FEDERATE_SAVE_AT":
        invoke_federate_callback(
            ...
            parts[1],
            decode_logical_time(logical_time_hint or parts[2], parts[3]),
        )
        return
    ...
    if kind == "ATTRIBUTE_IS_NOT_OWNED":
        invoke_federate_callback(ambassador, "attributeIsNotOwned", ObjectInstanceHandle(int(parts[1])), AttributeHandle(int(parts[2])))
        return
    ...
    if kind == "CONFIRM_ATTRIBUTE_OWNERSHIP_ACQUISITION_CANCELLATION":
        ...
        return
    raise RTIinternalError(f"Unknown hosted callback payload: {parts!r}")


__all__ = ["dispatch_hosted_callback"]
