...
class PublisherFederate(FederateAmbassadorSpec):
    ...
    def setup(self, rti: Any) -> None:
        self.rti = rti
        self.object_class = rti.get_object_class_handle(DEMO_OBJECT_CLASS)
        self.message_attr = rti.get_attribute_handle(self.object_class, MESSAGE_ATTRIBUTE)
        self.interaction_class = rti.get_interaction_class_handle(DEMO_INTERACTION_CLASS)
...
def run_minimal_demo_scenario(
    ...
    *,
    ...
    publisher_rti.join_federation_execution("Publisher", "minimal-demo-publisher", federation_name)
    subscriber_rti.join_federation_execution("Subscriber", "minimal-demo-subscriber", federation_name)
