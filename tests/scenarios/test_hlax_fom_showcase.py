...
from hla.verification.repo_internal.verification.hlax_fom_showcase import (
...
def test_hlax_fom_showcase_completes_all_example_federations() -> None:
    summary = run_hlax_fom_showcase(target_radar_steps=2)

    assert summary["suite_name"] == "hlax-fom-simulation-showcase"
    ...
    assert by_scenario["message-test"]["fom_modules"] == ["HLAx_Base.xml", "HLAx_MessageTest.xml"]
    assert by_scenario["space-lite"]["fom_modules"] == ["HLAx_Base.xml", "HLAx_SpaceLite.xml"]
    assert by_scenario["time-mgmt-test"]["fom_modules"] == ["HLAx_Base.xml", "HLAx_TimeMgmtTest.xml"]
    assert by_scenario["time-mgmt-test"]["delivered_tags"][-2:] == ["event-1", "event-2"]
    assert len(by_scenario["target-radar"]["track_reports"]) == 2


def test_hlax_fom_showcase_artifacts_are_generated(tmp_path) -> None:
    paths = write_hlax_fom_showcase_artifacts(tmp_path, target_radar_steps=1)
    ...
    assert "HLA-X FOM Simulation Showcase" in report
    ...
    assert manifest["schema_version"] == "hlax-hero-charts-v0.1"
...
def test_hlax_fom_showcase_ci_wrapper_bootstraps_source_checkout(tmp_path: Path) -> None:
    env = {"PATH": os.environ.get("PATH", ""), "HOME": os.environ.get("HOME", "")}
    output_dir = tmp_path / "analysis" / "hlax_fom_showcase"
    result = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "ci" / "hlax_fom_showcase.sh"),
            "--output-dir",
            str(output_dir),
            "--steps",
            "1",
    ...
    summary_path = output_dir / "hlax_fom_showcase_summary.json"
