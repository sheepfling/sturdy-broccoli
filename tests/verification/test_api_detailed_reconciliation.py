...
RECONCILIATION_REL = "requirements/2010/hla1516_1_api_detailed_reconciliation.csv"
OWNER_DOC = "docs/requirements/ieee-1516-2010/api_binding_bounded_family.md"
...
def test_api_detailed_reconciliation_spot_checks_key_rows():
    ...
    assert rows["HLA1516.1-CPP-4_2-CONNECT-001"].canonical_requirement_id == "HLA1516.1-FM-001"
    assert rows["HLA1516.1-CPP-4_2-CONNECT-001"].evidence_refs == (
        "tests/verification/test_spec_traceability_all_methods.py::test_all_generated_ambassador_methods_are_section_mapped",
        "tests/verification/test_requirements_ledger_v013.py::test_requirements_ledger_covers_generated_api_surface",
    )
    ...
    assert (
        "tests/factories/test_fom_time_factories.py::test_api_designator_metadata_preserves_standard_designator_types_across_java_and_cpp_bindings"
        in rows["HLA1516.1-API_DESIGNATORS-002"].evidence_refs
    )
    ...
    assert rows["HLA1516.1-CPP-12_12-RTIAMBASSADOR-234"].evidence_refs == (
        "tests/verification/test_api_detailed_reconciliation.py::test_selected_cpp_catalog_rows_are_declared_in_the_official_2010_cpp_api_headers",
    )
    ...
    assert rows["HLA1516.1-API_CPP-012"].mapping_notes.startswith(
    ...
    assert rows["HLA1516.1-WSDL-OP-001"].evidence_refs == (
        "tests/verification/test_imported_hla_packet_v1_0.py::test_imported_packet_extended_catalog_families_match_expected_shape",
    )


def test_api_detailed_reconciliation_is_explicitly_classified_as_mapping_bridge() -> None:
...
def _assert_selected_cpp_catalog_rows_are_declared_in_the_official_2010_cpp_api_headers() -> None:
...
def test_selected_cpp_catalog_rows_are_declared_in_the_official_2010_cpp_api_headers() -> None:
    _assert_selected_cpp_catalog_rows_are_declared_in_the_official_2010_cpp_api_headers()
