...
def test_omt_detailed_reconciliation_spot_checks_key_rows():
    ...
    assert rows["HLA1516.2-NORMALIZATION-030"]["current_status"] == "mapped"
    assert rows["HLA1516.2-OMT-OMT_NORM_NORMALIZATION-027"]["current_status"] == "mapped"
    assert "assess_omt_conformance" in rows["HLA1516.2-NORMALIZATION-030"]["notes"]
    assert "common Annex B normalization subset" in rows["HLA1516.2-NORMALIZATION-030"]["notes"]
    assert "normalize_dimension_value" in rows["HLA1516.2-NORMALIZATION-030"]["notes"]
    assert "assess_omt_conformance" in rows["HLA1516.2-OMT-OMT_NORM_NORMALIZATION-027"]["notes"]
    assert "common Annex B normalization subset" in rows["HLA1516.2-OMT-OMT_NORM_NORMALIZATION-027"]["notes"]
    assert "normalize_dimension_value" in rows["HLA1516.2-OMT-OMT_NORM_NORMALIZATION-027"]["notes"]
