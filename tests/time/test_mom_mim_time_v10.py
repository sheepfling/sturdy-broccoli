...
def test_timestamp_order_next_message_requests_deliver_in_time_order_and_query_galt_lits():
    ...
    assert initial_galt.time_is_valid and initial_galt.time == factory.makeTime(1.0)
...
def test_time_advance_request_waits_at_galt_boundary_but_available_request_can_grant_equal_galt():
    ...
    receiver.timeAdvanceRequest(factory.makeTime(5.0))
    _drain(sender, receiver)
    ...
    sender2.enableTimeRegulation(factory2.makeInterval(1.0))
    receiver2.enableTimeConstrained()
    _drain(sender2, receiver2)
    sender2.timeAdvanceRequest(factory2.makeTime(4.0))
    _drain(sender2, receiver2)
    receiver2.timeAdvanceRequestAvailable(factory2.makeTime(5.0))
    _drain(sender2, receiver2)
    assert receiver2_fed.last_callback("timeAdvanceGrant").args[0] == factory2.makeTime(5.0)
...
def test_retract_removes_queued_tso_message_before_grant():
    ...
    retraction = sender.sendInteraction(interaction, {track_id: b"withdrawn"}, b"withdraw", factory.makeTime(2.0))
