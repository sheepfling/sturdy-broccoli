...
def test_timestamp_order_attribute_updates_are_released_in_time_order():
    ...
    r1.enableTimeRegulation(tf.makeInterval(0.0))
    r2.enableTimeConstrained()
    drain(r1, r2)

    ret10 = r1.updateAttributeValues(obj, {position: b"t10"}, b"late", tf.makeTime(10.0))
    ret05 = r1.updateAttributeValues(obj, {position: b"t05"}, b"early", tf.makeTime(5.0))
    ...
    r1.timeAdvanceRequestAvailable(tf.makeTime(10.0))
    r2.timeAdvanceRequestAvailable(tf.makeTime(10.0))
    drain(r1, r2)
    ...
    assert f2.callbacks_named("timeAdvanceGrant")[-1].args[0] == tf.makeTime(10.0)
...
def test_retracted_timestamp_order_message_is_not_delivered():
    ...
    ret = r1.updateAttributeValues(obj, {position: b"withdraw"}, b"ret", tf.makeTime(5.0))
    assert ret is not None
    r1.retract(ret.handle)
    with pytest.raises(MessageCanNoLongerBeRetracted):
        r1.retract(ret.handle)

    r1.timeAdvanceRequestAvailable(tf.makeTime(5.0))
    r2.timeAdvanceRequestAvailable(tf.makeTime(5.0))
    drain(r1, r2)
...
def test_time_state_errors_and_zero_lookahead_are_enforced():
    ...
    with pytest.raises(TimeRegulationAlreadyEnabled):
        r1.enableTimeRegulation(tf.makeInterval(0.0))
    r1.modifyLookahead(tf.makeInterval(0.0))
    r1.timeAdvanceRequest(tf.makeTime(4.0))
    drain(r1)
    with pytest.raises(LogicalTimeAlreadyPassed):
        r1.timeAdvanceRequest(tf.makeTime(3.0))
...
def test_query_lookahead_requires_time_regulation_and_modify_lookahead_rejects_pending_advance():
    ...
    r1.enableTimeRegulation(tf.makeInterval(1.0))
    with pytest.raises(TimeRegulationIsNotEnabled):
        r2.queryLookahead()

    r2.enableTimeRegulation(tf.makeInterval(1.0))
    r2.enableTimeConstrained()
    drain(r1, r2)
...
def test_negative_lookahead_is_rejected_for_regulation_and_modification():
    ...
    r1.enableTimeRegulation(tf.makeInterval(1.0))
    with pytest.raises(InvalidLookahead):
        r1.modifyLookahead(tf.makeInterval(-0.5))
    ...
    r2.timeAdvanceRequest(tf.makeTime(5.0))
    with pytest.raises(InTimeAdvancingState):
        r2.modifyLookahead(tf.makeInterval(-0.5))
...
def test_section8_core_time_management_surface_covers_callbacks_states_and_grants():
    ...
    assert r1.queryLogicalTime() == tf.makeTime(0.0)
    assert r2.queryLogicalTime() == tf.makeTime(0.0)

    r1.enableTimeRegulation(tf.makeInterval(1.0))
    r2.enableTimeConstrained()
    drain(r1, r2)
    assert f1.last_callback("timeRegulationEnabled").args[0] == tf.makeTime(0.0)
    assert f2.last_callback("timeConstrainedEnabled").args[0] == tf.makeTime(0.0)
    assert r1.queryLookahead() == tf.makeInterval(1.0)
    ...
    r1.timeAdvanceRequest(tf.makeTime(4.0))
    r2.timeAdvanceRequestAvailable(tf.makeTime(4.0))
    drain(r1, r2)
    assert f1.last_callback("timeAdvanceGrant").args[0] == tf.makeTime(4.0)
    assert f2.last_callback("timeAdvanceGrant").args[0] == tf.makeTime(4.0)
    assert r1.queryLogicalTime() == tf.makeTime(4.0)
...
def test_section8_next_message_request_grants_earliest_tso_message():
    ...
    r1.updateAttributeValues(obj, {attr: b"early"}, b"early", tf.makeTime(2.0))
    r1.updateAttributeValues(obj, {attr: b"late"}, b"late", tf.makeTime(3.0))
    drain(r1, r2)
    ...
    r2.nextMessageRequest(tf.makeTime(10.0))
    drain(r1, r2)
    first = f2.last_callback("reflectAttributeValues")
    assert first is not None
    assert first.args[1][attr] == b"early"
    assert f2.last_callback("timeAdvanceGrant").args[0] == tf.makeTime(2.0)
    ...
    assert f2.last_callback("timeAdvanceGrant").args[0] == tf.makeTime(3.0)
