...
def joined_pair(name: str):
    ...
    r1.join_federation_execution("Sender", "Producer", name)
    r2.join_federation_execution("Receiver", "Consumer", name)
    return r1, r2, f1, f2


def setup_target_object(r1, r2):
    target = r1.get_object_class_handle("HLAobjectRoot.Target")
    position = r1.get_attribute_handle(target, "Position")
    r1.publish_object_class_attributes(target, {position})
    r2.subscribe_object_class_attributes(target, {position})
    obj = r1.register_object_instance(target, "Target-TSO")
    drain(r1, r2)
    return obj, position
...
def test_standard_mim_is_loaded_and_exposes_mom_classes_and_interactions():
    ...
    federation_class = r1.get_object_class_handle(mom.MOM_FEDERATION_OBJECT_CLASS)
    federate_class = r1.get_object_class_handle(mom.MOM_FEDERATE_OBJECT_CLASS)
    fed_name_attr = r1.get_attribute_handle(federation_class, "HLAfederationName")
    federate_name_attr = r1.get_attribute_handle(federate_class, "HLAfederateName")
    request = r1.get_interaction_class_handle(f"{mom.MOM_FEDERATION_INTERACTION_ROOT}.HLArequest.HLArequestFOMmoduleData")
    report = r1.get_interaction_class_handle(f"{mom.MOM_FEDERATION_INTERACTION_ROOT}.HLAreport.HLAreportFOMmoduleData")
    indicator_param = r1.get_parameter_handle(request, "HLAFOMmoduleIndicator")
    data_param = r1.get_parameter_handle(report, "HLAFOMmoduleData")
...
def test_mom_objects_can_be_discovered_and_attribute_values_requested():
    ...
    federation_class = r1.get_object_class_handle(mom.MOM_FEDERATION_OBJECT_CLASS)
    federation_name_attr = r1.get_attribute_handle(federation_class, "HLAfederationName")

    r1.subscribe_object_class_attributes(federation_class, {federation_name_attr})
    drain(r1)
    discoveries = f1.callbacks_named("discoverObjectInstance")
    assert discoveries
    mom_federation_object = discoveries[-1].args[0]

    r1.request_attribute_value_update(mom_federation_object, {federation_name_attr}, b"mom-query")
    drain(r1)
...
def test_mom_request_interaction_produces_report_interaction_for_subscribers():
    ...
    request = r1.get_interaction_class_handle(request_name)
    report = r1.get_interaction_class_handle(report_name)
    indicator = r1.get_parameter_handle(request, "HLAFOMmoduleIndicator")
    data = r1.get_parameter_handle(report, "HLAFOMmoduleData")

    r1.subscribe_interaction_class(report)
    r1.send_interaction(request, {indicator: b"0"}, b"mom-request")
    drain(r1)
...
def test_timestamp_order_attribute_updates_are_released_in_time_order():
    ...
    tf = r1.get_time_factory()

    r1.enable_time_regulation(tf.make_interval(0.0))
    r2.enable_time_constrained()
    drain(r1, r2)

    ret10 = r1.update_attribute_values(obj, {position: b"t10"}, b"late", tf.make_time(10.0))
    ret05 = r1.update_attribute_values(obj, {position: b"t05"}, b"early", tf.make_time(5.0))
    ...
    r1.time_advance_request_available(tf.make_time(10.0))
    r2.time_advance_request_available(tf.make_time(10.0))
    drain(r1, r2)
...
def test_retracted_timestamp_order_message_is_not_delivered():
    ...
    ret = r1.update_attribute_values(obj, {position: b"withdraw"}, b"ret", tf.make_time(5.0))
    assert ret is not None
    r1.retract(ret.handle)
    with pytest.raises(MessageCanNoLongerBeRetracted):
        r1.retract(ret.handle)

    r1.time_advance_request_available(tf.make_time(5.0))
    r2.time_advance_request_available(tf.make_time(5.0))
    drain(r1, r2)
...
def test_time_state_errors_and_zero_lookahead_are_enforced():
    ...
    with pytest.raises(TimeRegulationAlreadyEnabled):
        r1.enable_time_regulation(tf.make_interval(0.0))
    r1.modify_lookahead(tf.make_interval(0.0))
    r1.time_advance_request(tf.make_time(4.0))
    drain(r1)
    with pytest.raises(LogicalTimeAlreadyPassed):
        r1.time_advance_request(tf.make_time(3.0))

    r2 = rti_ambassador(engine=r1.backend.engine)
    f2 = RecordingFederateAmbassador()
    r2.connect(f2, CallbackModel.HLA_EVOKED)
    r2.join_federation_execution("Constrained", "Consumer", "time-errors-fed")
    r2.enable_time_constrained()
...
def test_query_lookahead_requires_time_regulation_and_modify_lookahead_rejects_pending_advance():
    ...
    r1.enable_time_regulation(tf.make_interval(1.0))
    with pytest.raises(TimeRegulationIsNotEnabled):
        r2.query_lookahead()

    r2.enable_time_regulation(tf.make_interval(1.0))
    r2.enable_time_constrained()
    drain(r1, r2)
...
def test_negative_lookahead_is_rejected_for_regulation_and_modification():
    ...
    r1.enable_time_regulation(tf.make_interval(1.0))
    with pytest.raises(InvalidLookahead):
        r1.modify_lookahead(tf.make_interval(-0.5))
    ...
    r2.time_advance_request(tf.make_time(5.0))
    with pytest.raises(InTimeAdvancingState):
        r2.modify_lookahead(tf.make_interval(-0.5))
...
def test_section8_core_time_management_surface_covers_callbacks_states_and_grants():
    ...
    assert r1.query_logical_time() == tf.make_time(0.0)
    assert r2.query_logical_time() == tf.make_time(0.0)
    ...
    assert r1.query_lookahead() == tf.make_interval(1.0)
    ...
    r1.time_advance_request(tf.make_time(4.0))
    r2.time_advance_request_available(tf.make_time(4.0))
    drain(r1, r2)
...
def test_section8_next_message_request_grants_earliest_tso_message():
    ...
    r2.subscribe_object_class_attributes(cls, {attr})
    r1.enable_time_regulation(tf.make_interval(1.0))
    r2.enable_time_constrained()
    drain(r1, r2)

    obj = r1.register_object_instance(cls, "Section8-NMR")
    drain(r1, r2)
    f2.clear()

    r1.update_attribute_values(obj, {attr: b"early"}, b"early", tf.make_time(2.0))
    r1.update_attribute_values(obj, {attr: b"late"}, b"late", tf.make_time(3.0))
    drain(r1, r2)
    ...
    r2.next_message_request(tf.make_time(10.0))
    drain(r1, r2)
    ...
    r2.resign_federation_execution(ResignAction.NO_ACTION)
    r1.destroy_federation_execution("section8-nmr-fed")
