...
def test_generate_fom_roundtrip_writes_explicit_2010_and_2025_artifacts(tmp_path: Path) -> None:
    ...
    assert target_radar_row.edition_scope == "both"
    ...
    assert report_2025.module_reports[0].edition_scope == "2025 only"
    ...
    assert "edition_scope" in payload["module_reports"][0]
    ...
    assert "Edition Scope" in direct_md_text
