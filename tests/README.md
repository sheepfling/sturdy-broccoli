...
- `tests/verification/`: conformance, requirements-ledger, MOM negative-matrix, spec-traceability, and downstream closeout-reporting tests. Use these for defended compliance work, not first-run validation.
- `tests/requirements/`: canonical 2025 requirement-owner, route-parity, backend-resolution, and imported-packet checks for the main `python1516_2025` RTI lane. Read these as requirement-surface tests, not as plan/worklist truth.
