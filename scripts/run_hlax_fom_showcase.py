...
def _validate_artifacts(output_dir: Path) -> list[str]:
    ...
    if not manifest_path.exists():
        ...
        if manifest.get("schema_version") != "hlax-hero-charts-v0.1":
            errors.append("chart manifest schema_version is not hlax-hero-charts-v0.1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run the HLA-X v0.1 FOM simulation showcase and write chart-ready artifacts.")
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "analysis" / "hlax_fom_showcase"),
    ...
    if errors:
        for error in errors:
            print(f"hlax fom showcase failed postcondition: {error}", file=sys.stderr)
        return 1
    return 0
