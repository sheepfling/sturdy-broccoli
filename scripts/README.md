...
- `./tools/fom-siso-runtime-surface-matrix` canonical scenario x surface integration packet for runtime-only, observer/visualizer, and observer/visualizer/bridge rows over the SISO showcase manifest
  - writes per-row observer API snapshots, bridge contract snapshots, an aggregate HTML artifact index, and optional screenshot galleries for selected SISO rows
  - named presets now cover bounded micro bridge hydration, heaviest 10-federate rows, and larger 5/10-federate showcase hydration without hand-selecting rows
