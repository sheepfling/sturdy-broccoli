...
def _attach_method_metadata(
    ...
    *,
    ...
    method.spec_reference = ref  # type: ignore[attr-defined]
    method.spec_source_summary = source_summary  # type: ignore[attr-defined]
    if abstract:
        method.__isabstractmethod__ = True  # type: ignore[attr-defined]
    return method


def _make_rti_abstract_method(method_name: str) -> Callable[..., Any]:
    snake_name = lower_camel_to_snake(method_name)

    @abstractmethod
    def _method(self, *args: Any, **kwargs: Any) -> Any:
        raise NotImplementedError

    return _attach_method_metadata(_method, method_name, snake_name, abstract=True, owner_name="RTIambassadorSpec")


def _make_rti_camel_alias(method_name: str) -> Callable[..., Any]:
    snake_name = lower_camel_to_snake(method_name)
...
def _make_callback_method(method_name: str) -> Callable[..., Any]:
    snake_name = lower_camel_to_snake(method_name)
...
def _make_callback_alias(method_name: str) -> Callable[..., Any]:
    snake_name = lower_camel_to_snake(method_name)
