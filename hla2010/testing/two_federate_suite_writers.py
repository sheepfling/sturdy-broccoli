...
def _write_track_csv(path: Path, reports: list[dict[str, Any]]) -> Path:
...
def _write_markdown(path: Path, summary: Mapping[str, Any], paths: SuitePaths) -> Path:
    ...
    lines = [
        ...
        f"- track reports: `{len(target_radar['track_reports'])}`",
        "",
        "## Profiles",
        "",
