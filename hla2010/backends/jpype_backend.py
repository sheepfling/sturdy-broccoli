"""JPype bridge for Java HLA RTI implementations.

This module is optional.  It imports JPype only when a JPype backend is created,
so the base package can be installed without Java bridge dependencies.
"""
...
from ..java_runtime import ensure_java_home
...
class JPypeBridge(JavaBridge):
    ...
    def byte_array(self, data: bytes) -> Any:
        ...
        # Java bytes are signed.  JPype accepts values in -128..127 reliably.
    ...
    def _factory_collection(self, rti_ambassador: Any, factory_method: str, values: Sequence[Any], *, capacity: int | None = None) -> Any:
        ...
        try:
            ...
            if rti_ambassador is not None:
                try:
                    ...
                    pass
            # The bundled Java shim exposes concrete classes in hla.rti1516e.
    ...
    def exception_class_name(self, exc: BaseException) -> str | None:
        # JPype Java exceptions expose a Java class through getClass().
...
def create_jpype_backend(config: JPypeConfig = JPypeConfig()) -> JPypeRTIBackend:
    ...
    The RTI vendor jars/classes must be available on the configured classpath.
    """
    ...
    return JPypeRTIBackend(java_rti_ambassador=java_rti, bridge=bridge, info=info)
...
__all__ = ["JPypeBridge", "JPypeConfig", "JPypeRTIBackend", "create_jpype_backend", "rti_ambassador"]
