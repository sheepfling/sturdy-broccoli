...
- generate the scenario x surface matrix for runtime-only, observer/visualizer, and observer/visualizer/bridge rows: `./tools/fom-siso-runtime-surface-matrix`
  ...
  - use `--preset micro-bridge-smoke` for bounded 2-federate bridge hydration, `--preset showcase-hydrated` for 5/10-federate visualizer hydration, `--preset heaviest-interesting` for the 10-federate Link16/RPR/Space rows, or `--preset stress-mixed` for a mixed runtime-plus-visualizer stress packet
  - the packet now also writes `siso_runtime_surface_matrix_index.html` so the per-row hydrated artifacts can be browsed from one page
