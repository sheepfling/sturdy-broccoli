...
`hla1516_1_conf_detailed_reconciliation.csv` is the companion bridge for the imported IEEE 1516.1 Clause 13 conformance-package rows. It does not claim external certification. Instead, it records that the current repo's generated Clause 13 conformance packet, harmonization index, ambassador traceability, explicit requirement markers, packet verification, and verification-package artifacts directly back the imported federate and RTI conformance-package rows as reviewable repo-native evidence.
...
`hla1516_2_omt_detailed_reconciliation.csv` is the whole-family companion bridge for imported IEEE 1516.2 OMT master rows. It lifts the current 1516.2 clause, parser, merge, lexicon, conformance-boundary, and annex evidence onto the imported OMT-family seeds and clause-detail rows. Rows with direct parser, merge, or fixture evidence are `mapped`, broad introductory and documentation-heavy rows remain intentionally `partial`, and the former Annex B normalization tail is now also `mapped` because the repo proves parser and metadata-preservation handling plus executable support for the carried common normalization subset through `hla.rti-core` `normalize_dimension_value`, `assess_omt_conformance`, and the standard-MIM normalization services already exercised by the Python RTI support-service lane.
